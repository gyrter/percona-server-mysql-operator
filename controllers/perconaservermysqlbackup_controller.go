...
func (r *PerconaServerMySQLBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if cluster.Spec.Backup == nil {
		return rr, errors.New("spec.backup stanza is required in PerconaServerMySQL custom resource")
	}

	storage, ok := cluster.Spec.Backup.Storages[cr.Spec.StorageName]
	if !ok {
		return rr, errors.Errorf("%s not found in spec.backup.storages in PerconaServerMySQL custom resource", cr.Spec.StorageName)
	}
